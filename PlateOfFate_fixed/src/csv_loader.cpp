...
std::vector<Customer> LoadCustomers(const std::string& path,
  ...
  while (std::getline(file, line)) {
    ...
    if (type_str == "single")      customer.type = CUSTOMER_SINGLE;
    else if (type_str == "group")  customer.type = CUSTOMER_GROUP;
    else                           customer.type = CUSTOMER_VIP;
    ...
    if (customer.floor == 1)      customer.patience = config.floor1_patience;
    else if (customer.floor == 2) customer.patience = config.floor2_patience;
    else                          customer.patience = config.floor3_patience;
